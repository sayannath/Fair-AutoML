    ...
    def __init__(
        ...
        if X.shape[0] != y.shape[0]:
            raise ValueError(
                ...
                "datapoints, but have %d and %d." % (X.shape[0], y.shape[0])
            )
        if self.feat_type is None:
            self.feat_type = ["Numerical"] * X.shape[1]
        if X.shape[1] != len(self.feat_type):
            raise ValueError(
                "X and feat_type must have the same number of columns, "
                "but are %d and %d." % (X.shape[1], len(self.feat_type))
            )
