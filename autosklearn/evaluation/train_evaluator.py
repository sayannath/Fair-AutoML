...
baseCrossValidator_defaults: Dict[str, Dict[str, Optional[Union[int, float, str]]]] = {
    ...
    "GroupShuffleSplit": {"n_splits": 5, "test_size": None, "random_state": None},
    "StratifiedShuffleSplit": {"n_splits": 10, "test_size": None, "random_state": None},
    "ShuffleSplit": {"n_splits": 10, "test_size": None, "random_state": None},
}
...
def _fit_with_budget(
    ...
    if (
        ...
        if model.estimator_supports_iterative_fit():
            ...
            model.iterative_fit(
                Xt, Y_train[train_indices], n_iter=n_iter, refit=True, **fit_params
            )
        else:
            _fit_and_suppress_warnings(
                logger,
...
class TrainEvaluator(AbstractEvaluator):
    ...
    def _partial_fit_and_predict_iterative(
        ...
        if model.estimator_supports_iterative_fit():
            ...
            while (
                ...
                model.iterative_fit(
                    Xt, self.Y_train[train_indices], n_iter=n_iter, **fit_params
                )
                (Y_train_pred, Y_optimization_pred, Y_valid_pred, Y_test_pred) = (
                    self._predict(
                        model,
                        train_indices=train_indices,
                        test_indices=test_indices,
    ...
    def _predict(
        ...
        if self.X_test is not None:
            ...
            test_pred = self.predict_function(
                X_test, model, self.task_type, self.Y_train[train_indices]
            )
        else:
            test_pred = None
    ...
    def get_splitter(
        ...
        if self.resampling_strategy is not None and not isinstance(
            ...
            if (
                issubclass(self.resampling_strategy, BaseCrossValidator)
                or issubclass(self.resampling_strategy, _RepeatedSplits)
                or issubclass(self.resampling_strategy, BaseShuffleSplit)
            ):
                ...
                if (
                    ...
                    D.info["task"] in REGRESSION_TASKS
                    and D.info["task"] != MULTIOUTPUT_REGRESSION
                ):
                ...
                if class_name == "PredefinedSplit":
                    if "test_fold" not in self.resampling_strategy_args:
                        raise ValueError(
                            "Must provide parameter test_fold"
                            " for class PredefinedSplit."
                        )
                if (
                    class_name == "LeaveOneGroupOut"
                    or class_name == "LeavePGroupsOut"
                    or class_name == "GroupKFold"
                    or class_name == "GroupShuffleSplit"
                ):
                    if "groups" not in self.resampling_strategy_args:
                        raise ValueError(
                            "Must provide parameter groups "
                            "for chosen CrossValidator."
                        )
                    try:
                        if (
                            np.shape(self.resampling_strategy_args["groups"])[0]
                            != y.shape[0]
                        ):
                            raise ValueError(
                                "Groups must be array-like " "with shape (n_samples,)."
                            )
                    except Exception:
                        raise ValueError(
                            "Groups must be array-like " "with shape (n_samples,)."
                ...
                for key in ref_arg_dict:
                    if key == "n_splits":
                        if "folds" not in self.resampling_strategy_args:
                            self.resampling_strategy_args["folds"] = ref_arg_dict[
                                "n_splits"
                            ]
                    else:
                ...
                if shuffle:
                    try:
                        cv = StratifiedShuffleSplit(
                            n_splits=1, test_size=test_size, random_state=1
                        )
