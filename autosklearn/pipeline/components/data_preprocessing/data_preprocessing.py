...
from autosklearn.pipeline.components.data_preprocessing.data_preprocessing_categorical import (
    CategoricalPreprocessingPipeline,
)
from autosklearn.pipeline.components.data_preprocessing.data_preprocessing_numerical import (
...
class DataPreprocessor(AutoSklearnComponent):
    ...
    def __init__(
        ...
        if categorical_features is not None:
            ...
            if categorical_features.dtype != "bool":
                raise ValueError(
                    "Parameter categorical_features must" " only contain booleans."
                )
        self.config = config
    ...
    def _get_hyperparameter_search_space_recursevely(
        dataset_properties, cs, transformer
    ):
