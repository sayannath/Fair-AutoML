...
class ARDRegression(AutoSklearnRegressionAlgorithm):
    ...
    def fit(self, X, Y):
        ...
        self.estimator = sklearn.linear_model.ARDRegression(
            n_iter=self.n_iter,
            tol=self.tol,
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        alpha_2 = UniformFloatHyperparameter(
            name="alpha_2", log=True, lower=10**-10, upper=10**-3, default_value=10**-6
        )
        lambda_1 = UniformFloatHyperparameter(
            name="lambda_1", log=True, lower=10**-10, upper=10**-3, default_value=10**-6
        )
        lambda_2 = UniformFloatHyperparameter(
            name="lambda_2", log=True, lower=10**-10, upper=10**-3, default_value=10**-6
        )
