...
def parse_include_exclude_components(
    ...
    if include_preprocessors is not None and exclude_preprocessors is not None:
        raise ValueError(
            "Cannot specify include_preprocessors and " "exclude_preprocessors."
        )
    ...
    if include_estimators is not None and exclude_estimators is not None:
        raise ValueError("Cannot specify include_estimators and " "exclude_estimators.")
...
def _get_regression_configuration_space(
    info: Dict[str, Any], include: Dict[str, List[str]], exclude: Dict[str, List[str]]
) -> ConfigurationSpace:
    task_type = info["task"]
    sparse = False
    multioutput = False
