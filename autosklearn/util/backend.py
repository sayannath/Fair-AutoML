...
class BackendContext(object):
    ...
    def __init__(
        ...
        if temporary_directory == output_directory and temporary_directory is not None:
            raise ValueError(
                "The temporary and the output directory " "must be different."
            )
    ...
    def delete_directories(self, force: bool = True) -> None:
        if self.output_directory and (
            self.delete_output_folder_after_terminate or force
        ):
            if self._output_dir_created is False:
                raise ValueError(
                    "Failed to delete output dir: %s because auto-sklearn did not "
        ...
        if self.delete_tmp_folder_after_terminate or force:
            if self._tmp_dir_created is False:
                raise ValueError(
                    ...
                    "exist when instantiating auto-sklearn." % self.temporary_directory
                )
            try:
    ...
    * true targets of the ensemble
    ...
    def __init__(self, context: BackendContext):
        ...
        if self.output_directory is not None:
            if not os.path.exists(self.output_directory):
                raise ValueError(
                    "Output directory %s does not exist." % self.output_directory
                )
    ...
    def save_start_time(self, seed: str) -> str:
        ...
        if not isinstance(start_time, float):
            raise ValueError(
                "Start time must be a float, but is %s." % type(start_time)
            )
    ...
    def get_numrun_directory(self, seed: int, num_run: int, budget: float) -> str:
        return os.path.join(
            self.internals_directory, "runs", "%d_%d_%s" % (seed, num_run, budget)
        )
    ...
    def list_all_models(self, seed: int) -> List[str]:
        ...
        model_files = glob.glob(
            os.path.join(
                glob.escape(runs_directory), "%d_*" % seed, "%s.*.*.model" % seed
            )
        )
        return model_files

    def load_models_by_identifiers(
        self, identifiers: List[Tuple[int, int, float]]
    ) -> Dict:
        models = dict()
    ...
    def load_model_by_seed_and_id_and_budget(
    ...
    def save_numrun_to_dir(
        ...
        if cv_model is not None:
            file_path = os.path.join(
                tmpdir, self.get_cv_model_filename(seed, idx, budget)
            )
            with open(file_path, "wb") as fh:
