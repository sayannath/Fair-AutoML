...
now = now.replace(":", "_")
temp_path = "mep_RF_SPD_race" + str(now)
try:
...
a_file = open("mep_rf_spd_60sp" + str(now) + ".pkl", "wb")
pickle.dump(automl.cv_results_, a_file)
a_file.close()

a_file1 = open("automl_mep_rf_spd_60sp" + str(now) + ".pkl", "wb")
...
print("SPD-Accuracy score:", sklearn.metrics.accuracy_score(y_test_full, predictions))
...
write_file(
    "./run_history/mep_rf_spd_race_run_history.json",
