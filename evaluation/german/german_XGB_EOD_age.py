...
now = now.replace(":", "_")
temp_path = "german_xgb_eod" + str(now)
try:
...
class CustomXGBoost(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        n_estimators = UniformIntegerHyperparameter(
            "n_estimators", 210, 781, default_value=210
        )
        max_depth = UniformIntegerHyperparameter("max_depth", 3, 9, default_value=6)
        learning_rate = UniformFloatHyperparameter(
            "learning_rate", 0.23749, 0.77773, default_value=0.3
        )
        subsample = UniformFloatHyperparameter(
            "subsample", 0.27425, 0.72543, default_value=0.72543
        )
...
write_file(
    "./run_history/german_xgb_eod_age_run_history.json",
